# Promoter region detection (all types)

Name: `DNA/promoter-all`
Category: Promoter
...
Promoter region detection (all types) from the Nucleotide Transformer downstream tasks benchmark.
Sequences are 300bp DNA sequences. The task is binary classification.

**Source:** InstaDeepAI/nucleotide_transformer_downstream_tasks (HuggingFace), config `promoter_all`

## Data Format

- **Input:** DataFrame with columns:
  - `sequence`: DNA sequence string (300bp)
...
task = gl.BenchmarkTask(name='DNA/promoter-all', fold='0', user='your_name')
